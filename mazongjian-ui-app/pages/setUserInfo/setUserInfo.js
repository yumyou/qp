...
Page({
  ...
  setPhone: function(){
    ...
    wx.navigateTo({
      url: '../setUserPhone/setUserPhone?phone='+phone+'&direct=true',
      events: {
        // 子页完成后触发，直接用新接口免验证码更新
        onPhoneSet: function(data){
          if(data && data.mobile){
            that.updateMobileDirect(data.mobile)
          }
        }
      }
    })
  ...
  // 免验证码直接修改手机号
  updateMobileDirect:function(mobile){
    var that = this;
    if(!mobile){
    ...
    http.request(
      "/member/user/update-mobile-direct?mobile="+encodeURIComponent(mobile),
      "1",
      "post",
      {},
      app.globalData.userDatatoken.accessToken,
      ...
      function success(info) {
        if (info.code == 0) {
          ...
          wx.showToast({ title: info.msg || '修改失败', icon: 'none' })
        }
      },
      function fail(info) {
