...
Page({
  ...
  submit: function(){
    ...
    if (!this.data.roomName) {
      missingFields.push('房间名称')
    }
    if (!this.data.roomCallName) {
      missingFields.push('房间简称')
    }
    if (!this.data.type) {
      missingFields.push('房间类型')
    }
    if (!this.data.price) {
      missingFields.push('房间单价')
    }
    if (!this.data.tongxiaoPrice) {
      missingFields.push('通宵价格')
    }
    if (!this.data.minHour) {
      missingFields.push('最低下单小时')
    }
    if (!this.data.leadHour) {
      missingFields.push('提前开始时间')
    }
    if (!this.data.leadDay) {
      missingFields.push('提前下单天数')
    }
    if (!this.data.label) {
      missingFields.push('房间标签')
    }
    if (!this.data.fileList.length) {
      missingFields.push('房间图片')
    }
    
    // 如果有未填写的字段，显示具体提示
    if (missingFields.length > 0) {
      let message = '请填写以下必填项：\n' + missingFields.join('、')
      wx.showModal({
        title: '信息不完整',
        content: message,
        showCancel: false,
