...
function blueDoorOpen(lockInfo){
    lockInfo = JSON.parse(lockInfo);
    const lockData = lockInfo.lockData;
...
function queryLockPwd(lockInfo){
  lockInfo = JSON.parse(lockInfo);
...
function setLockGateWay(lockInfo){
  lockInfo = JSON.parse(lockInfo);
  console.log('lockInfo',  lockInfo  );
  const lockData = lockInfo.lockData;
  wx.showLoading({ title: `请靠近门锁` });
...
function setLockPwd(lockInfo,passcode){
  lockInfo = JSON.parse(lockInfo);
  console.log('lockInfo',  lockInfo  );
  const lockData = lockInfo.lockData;
  wx.showLoading({ title: `请靠近门锁` });
...
function handleResetLock(lockInfo){
  lockInfo = JSON.parse(lockInfo);
  const lockData = lockInfo.lockData;
  console.log('lockInfo',  lockInfo  );
  setTimeout(() => {
