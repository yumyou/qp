...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getDeviceUseInfoPage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getDeviceUseInfo(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateDeviceUseInfo(this.form).then(response => {
        ...
        // 添加的提交
        createDeviceUseInfo(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除设备使用记录编号为"' + id + '"的数据项?').then(function() {
          return deleteDeviceUseInfo(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有设备使用记录数据项?').then(() => {
          this.exportLoading = true;
          return exportDeviceUseInfoExcel(params);
        }).then(response => {
          this.$download.excel(response, '设备使用记录.xls');
