...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getGroupPayInfoPage(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateGroupPayInfo(this.form).then(response => {
        ...
        // 添加的提交
        createGroupPayInfo(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除团购支付信息编号为"' + id + '"的数据项?').then(function() {
          return deleteGroupPayInfo(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有团购支付信息数据项?').then(() => {
          this.exportLoading = true;
          return exportGroupPayInfoExcel(params);
        }).then(response => {
          this.$download.excel(response, '团购支付信息.xls');
